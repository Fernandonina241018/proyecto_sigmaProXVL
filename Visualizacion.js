        ...
        try {
            ...
            switch (currentType) {
                ...
                case 'control': {
                 ...
                 case 'heatmap': {
                     const numCols = getNumericColumns(data);
                     if (numCols.length === 0) { alert('⚠️ No hay columnas numéricas para correlación.'); return; }
...
function _attachExportPanelListeners(predefinidos, controlsPanel) {
    ...
    // Getter público para ReporteManager
     ...
     function renderHeatmap(data, columnas) {
         ...
         for (let i = 0; i < n; i++) {
             for (let j = 0; j < n; j++) {
                 ...
                 ctx.fillText(
                     valor.toFixed(2),
                     padding.left + j * cellW + cellW / 2,
                     padding.top + i * cellH + cellH / 2
                 );
             }
         }
         ...
         columnas.forEach((col, i) => {
             ctx.save();
             ctx.translate(
                 padding.left + i * cellW + cellW / 2,
                 padding.top + plotH + 20
             );
     ...
     function renderViolin(data, columnas) {
         ...
         columnas.forEach((col, colIndex) => {
             ...
             for (let i = kde.xValues.length - 1; i >= 0; i--) {
                 ...
                 ctx.lineTo(
                     centerX + width,
                     padding.top + (1 - kde.xValues[i]) * plotH
                 );
             }
         ...
         for (let i = 0; i < n; i++) {
             for (let j = 0; j < n; j++) {
                 if (i === j) {
                     ...
                     const corr = calcularCorrelacionPearson(
                         getColumnValues(data, columnas[i]),
                         getColumnValues(data, columnas[j])
                     );
                     matrix[i][j] = corr;
     ...
     function calcularKDE(values) {
         if (values.length === 0) return { xValues: [], densities: [] };
