...
const UsuariosManager = (() => {
    ...
    function _renderTabla(usuarios) {
        ...
        if (!wrap)
            return;
        if (countEl)
            countEl.textContent = `${usuarios.length} usuario${usuarios.length !== 1 ? 's' : ''}`;
        if (!usuarios.length) {
    ...
    function _showResetPasswordModal(username) {
        ...
        document.getElementById('usr-reset-quick').addEventListener('click', async () => {
            if (!confirm('¿Resetear contraseña a "user0000"? El usuario podrá iniciar sesión con esa contraseña y deberá cambiarla al primer acceso.'))
                return;
...
function _getAvatarColor(nombre) {
    if (!nombre)
        return 'linear-gradient(135deg, #3046ac, #6366f1)';
