...
const Auth = (() => {
    ...
    function _getSession() {
        try {
            const r = sessionStorage.getItem(CFG.SESSION_STORAGE_KEY);
    ...
    function _resetActivityTimer() {
        ...
        _scheduleTimers();
        const w = document.getElementById('auth-timeout-warn');
        if (w)
            w.style.opacity = '0';
    }
    ...
    function _showTimeoutWarning() {
        ...
        const c = document.getElementById('auth-warn-countdown');
        if (c)
            c.textContent = secs;
        clearInterval(_countdownTimer);
        _countdownTimer = setInterval(() => { secs--; if (c)
            c.textContent = secs; if (secs <= 0)
            clearInterval(_countdownTimer); }, 1000);
    }
    function _expireSession() { _clearSession(); showLogin('timeout'); if (_onLogout)
        _onLogout('timeout'); }
    // ── Intentos ──────────────────────────
    ...
    function _renderParticles() {
        ...
        function updateSymbols() {
            for (let i = 0; i < particles.length; i++) {
                ...
                if (t < 0.12)
                    p.opacity = (t / 0.12) * 0.75;
                else if (t > 0.80)
                    p.opacity = ((1 - t) / 0.20) * 0.75;
                else
                    p.opacity = 0.75;
    ...
    let _lastActivity = 0;
    function _onActivity() { const now = Date.now(); if (now - _lastActivity < 10000)
        return; _lastActivity = now; _resetActivityTimer(); }
    // ── API pública ───────────────────────
    ...
    function logout() {
        ...
        if (token) {
            fetch(`${CFG.API_URL}/api/logout`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } }).catch(() => { });
        }
        _clearSession();
        _unregisterActivityListeners();
        showLogin('logout');
    ...
    function keepAlive() { _resetActivityTimer(); const w = document.getElementById('auth-timeout-warn'); if (w)
        w.style.opacity = '0'; }
