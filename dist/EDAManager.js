...
const EDAManager = (function () {
    ...
    function testNormalidadJB(values) {
        ...
        if (n < 3)
            return { JB: 0, p: 1, esNormal: true, skew: 0, kurt: 0 };
    ...
    function generarRecomendaciones({ numericCols, normalidad, outliers, correlacionesFuertes, descriptivas, missingCount, totalRows }) {
        ...
        if (correlacionesFuertes.length > 0) {
            const details = correlacionesFuertes.map(c => `${escHtml(c.col1)} ↔ ${escHtml(c.col2)} (r=${c.r.toFixed(3)})`).join('; ');
            recs.push({
                tipo: 'info',
                icono: '🔗',
    ...
    function renderCorrelacion(results) {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            for (let j = 0; j < cols.length; j++) {
                ...
                if (i === j)
                    cls = 'eda-corr-diagonal';
                else if (r >= 0.7)
                    cls = 'eda-corr-strong-pos';
                else if (r >= 0.4)
                    cls = 'eda-corr-moderate-pos';
                else if (r <= -0.7)
                    cls = 'eda-corr-strong-neg';
                else if (r <= -0.4)
                    cls = 'eda-corr-moderate-neg';
