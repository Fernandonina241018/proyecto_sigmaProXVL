...
function getSeccionesSidebar() {
    ...
    Object.keys(secciones).forEach(key => {
        if (secciones[key].options.length === 0)
            delete secciones[key];
    });
    return secciones;
}
...
function buildInterpretacion(nombre, significativo, valores) {
    ...
    return plantilla.replace(/\{(\w+)\}/g, (_, key) => valores[key] !== undefined ? valores[key] : `{${key}}`);
}
// ════════════════════════════════════════
