...
function switchView(viewName) {
    ...
    if (viewName === 'analisis') {
        ...
        if (imported && !ultimosResultados)
            displayImportedData(imported);
    }
    if (viewName === 'datos')
        DatosManager.buildView();
    if (viewName === 'visualizacion')
        inicializarVisualizacion();
    if (viewName === 'reportes')
        inicializarReportes();
    if (viewName === 'auditoria') {
        if (PermisosManager.protegerVista('auditoria-container', 'ver_auditoria'))
            inicializarAuditoria();
    }
    if (viewName === 'usuarios') {
        if (PermisosManager.protegerVista('usuarios-container', 'gestionar_usuarios'))
            inicializarUsuarios();
    }
...
function getEDAData() {
    ...
    return _normalizeToEDAFormat({ headers: sheet.headers, data: rawData, rowCount: rawData.length }, format);
}
/**
...
function _normalizeToEDAFormat(source, format) {
    ...
    if (format === 'array') {
        ...
        normalized = rawData.map(row => headers.map((_, i) => (row[i] !== undefined && row[i] !== null ? row[i] : '')));
    }
    else {
        // array-de-objetos → convertir a array-de-arrays
        normalized = rawData.map(row => headers.map(h => (row[h] !== undefined && row[h] !== null ? row[h] : '')));
    }
    return {
        headers,
        data: normalized,
...
function verificarParametrosAntesAnalisis(callback) {
    ...
    const numCols = imported.headers.filter((_, i) => imported.data.some(row => {
...
function normalizeData() {
    ...
    numericCols.forEach(col => {
        ...
        const range = max - min;
        if (range === 0)
            return; // columna constante, evitar división por cero
        imported.data.forEach(row => {
...
function removeNulls() {
    ...
    imported.data = imported.data.filter(row => imported.headers.every(header => {
...
function createCalculatedColumn() {
    ...
    if (!col1 || !numericCols.includes(col1)) {
        alert('⚠️ Columna no válida.');
        return;
    }
    const col2 = prompt(`Columna B (disponibles: ${numericCols.filter(c => c !== col1).join(', ')}):`);
    if (!col2 || !numericCols.includes(col2) || col2 === col1) {
        alert('⚠️ Columna no válida.');
        return;
    }
    const op = prompt('Operación: +, -, *, /');
    if (!['+', '-', '*', '/'].includes(op)) {
        alert('⚠️ Operación no válida. Usa +, -, * o /');
        return;
    }
...
function _renderUserChip(username) {
    ...
    if (userRole)
        userRole.textContent = isAdmin ? 'Administrador' : 'Usuario';
    // Actualizar dropdown
...
function mostrarModalConfiguracionHypothesis(statName) {
    ...
    const configs = {
        ...
        'Test de Kolmogorov-Smirnov': { title: '🔔 Configurar Test de Kolmogorov-Smirnov', catCols: 0, numCols: 1, catLabel: null, numLabel: 'Columna a analizar' },
        'Test de Anderson-Darling': { title: '🔔 Configurar Test de Anderson-Darling', catCols: 0, numCols: 1, catLabel: null, numLabel: 'Columna a analizar' },
        "Test de D'Agostino-Pearson": { title: "🔔 Configurar Test de D'Agostino-Pearson", catCols: 0, numCols: 1, catLabel: null, numLabel: 'Columna a analizar' },
        ...
        'Wilcoxon': { title: '⚖️ Configurar Test de Wilcoxon', catCols: 0, numCols: 2, catLabel: null, numLabel: 'Dos columnas pareadas (antes/después)', paired: true },
        'Friedman': { title: '📊 Configurar Test de Friedman', catCols: 1, numCols: 1, catLabel: 'Columna de Bloques (mínimo 3)', numLabel: 'Columna de Tratamientos' },
        'Test de Signos': { title: '✚ Configurar Test de Signos', catCols: 0, numCols: 2, catLabel: null, numLabel: 'Dos columnas pareadas (antes/después)', paired: true },
...
function abrirModalConfigCorrelacion(imported, statName) {
    ...
    document.getElementById('correlacion-modal-confirm').onclick = () => {
        ...
        const seleccionados = [];
        if (document.getElementById('corr-pearson')?.checked)
            seleccionados.push('pearson');
        if (document.getElementById('corr-spearman')?.checked)
            seleccionados.push('spearman');
        if (document.getElementById('corr-kendall')?.checked)
            seleccionados.push('kendall');
        if (document.getElementById('corr-covarianza')?.checked)
            seleccionados.push('covarianza');
...
function abrirModalConfigRegresion(imported, statName) {
    ...
    document.getElementById('regresion-modal-confirm').onclick = () => {
        ...
        const seleccionados = [];
        if (document.getElementById('reg-lineal')?.checked)
            seleccionados.push('lineal');
        if (document.getElementById('reg-polinomial')?.checked)
            seleccionados.push('polinomial');
        if (seleccionados.length === 0) {
...
function abrirModalConfigMetricasError(imported, statName) {
    ...
    document.getElementById('metricas-error-modal-confirm').onclick = () => {
        ...
        const seleccionados = [];
        if (document.getElementById('metrica-rmse')?.checked)
            seleccionados.push('rmse');
        if (document.getElementById('metrica-mae')?.checked)
            seleccionados.push('mae');
        if (document.getElementById('metrica-r2')?.checked)
            seleccionados.push('r2');
        if (seleccionados.length === 0) {
...
function abrirModalConfigPareto(imported) {
    ...
    document.getElementById('pareto-config-modal-confirm').onclick = () => {
        ...
        document.querySelectorAll('.menu-option').forEach(opt => {
            if (opt.textContent.trim() === 'Diagrama de Pareto')
                opt.classList.add('selected');
        });
...
function abrirModalConfigObsPred(imported, statName) {
    ...
    document.getElementById('obs-pred-confirm').addEventListener('click', () => {
        ...
        document.querySelectorAll('.menu-option').forEach(opt => {
            if (opt.textContent.trim() === statLabel)
                opt.classList.add('selected');
        });
...
function abrirModalConfigBootstrap(imported) {
    ...
    document.getElementById('bootstrap-config-modal-confirm').onclick = () => {
        ...
        document.querySelectorAll('.menu-option').forEach(opt => {
            if (opt.textContent.trim() === 'Bootstrap')
                opt.classList.add('selected');
        });
...
function mostrarAsistenteAnalisis() {
    ...
    function renderizar() {
        ...
        pregunta.opciones.forEach(opcion => {
            ...
            btn.onmouseout = function () { if (respuestaActual !== this.dataset.value)
        ...
        overlay.onclick = (e) => { if (e.target.id === 'asistente-modal') {
            resetAsistente();
            historial = [];
            overlay.remove();
        } };
