...
const DatosManager = (() => {
    ...
    function _doExport(format) {
        ...
        switch (format) {
            case 'csv':
                ...
                const csvLines = rows.map(row => row.map(val => {
    ...
    function _renderFilesArea(imported) {
        if (!imported)
            return '<p class="dm-no-file">No hay archivos cargados</p>';
    ...
    function _renderTable(imported) {
        ...
        if (_searchQuery) {
            ...
            data = data.filter(row => imported.headers.some(h => String(row[h] ?? '').toLowerCase().includes(q)));
        }
        ...
        if (infoEl)
            infoEl.textContent = `${start + 1}–${Math.min(start + _pageSize, total)} de ${total} filas`;
    ...
    function _refreshTable() {
        ...
        if (wrap)
            wrap.innerHTML = _renderTable(imported);
    ...
    function _attachListeners(imported) {
        ...
        const _doImport = () => {
            if (!PermisosManager.puede('importar_datos')) {
                PermisosManager.mostrarDenegado('importar_datos');
                return;
            }
        ...
        document.getElementById('dm-btn-clear-data')?.addEventListener('click', () => {
            if (!confirm('¿Eliminar los datos cargados?'))
                return;
        ...
        const _wrapEditar = (fn) => () => {
            if (!PermisosManager.puede('editar_datos')) {
                PermisosManager.mostrarDenegado('editar_datos');
                return;
            }
            fn();
        };
        ...
        if (imported)
            _attachParamsListeners(imported);
    }
    ...
    function _attachTableListeners() {
        ...
        document.querySelectorAll('.dm-th-rename').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (_sortCol === oldName)
                    _sortCol = newName;
                if (_outlierCols.has(oldName)) {
                    _outlierCols.delete(oldName);
                    _outlierCols.add(newName);
                }
    ...
    function _attachParamsListeners(imported) {
        ...
        document.getElementById('dm-params-reset')?.addEventListener('click', () => {
            if (!confirm('¿Limpiar todos los parámetros de control?'))
                return;
    ...
    function _doClean() {
        const imported = _getData();
        if (!imported)
            return _showToast('⚠️ No hay datos cargados', true);
        let n = 0;
        imported.data.forEach(row => imported.headers.forEach(h => {
    ...
    function _doRemoveNulls() {
        ...
        imported.data = imported.data.filter(row => imported.headers.every(h => {
            const v = row[h];
    ...
    function _doNormalize() {
        ...
        const numCols = _getNumericCols(imported);
        if (!numCols.length)
            return _showToast('⚠️ No hay columnas numéricas', true);
    ...
    function _doDetectOutliers() {
        ...
        numCols.forEach(col => {
            ...
            imported.data.forEach(row => {
                const v = parseFloat(row[col]);
                if (!isNaN(v) && (v < range.low || v > range.high))
                    totalOutliers++;
            });
        });
    ...
    function _doCreateColumn() {
        ...
        const numCols = _getNumericCols(imported);
        if (numCols.length < 2)
            return _showToast('⚠️ Se necesitan al menos 2 columnas numéricas', true);
    ...
    function _showCalculatedColumnModal(imported, numCols) {
        ...
        document.getElementById('dm-modal-confirm').addEventListener('click', () => {
            ...
            if (colA === colB && op === '/') {
                _showToast('⚠️ División de una columna por sí misma siempre es 1', true);
                return;
            }
            if (imported.headers.includes(newName)) {
                _showToast(`⚠️ Ya existe una columna llamada "${newName}"`, true);
                return;
            }
    ...
    function _doDuplicateColumn() {
        ...
        if (!imported.headers.length)
            return _showToast('⚠️ No hay columnas disponibles', true);
        ...
        document.getElementById('dm-dupcol-confirm').addEventListener('click', () => {
            ...
            if (!newName)
                return _showToast('⚠️ Escribe un nombre para la nueva columna', true);
            if (imported.headers.includes(newName))
                return _showToast(`⚠️ Ya existe una columna llamada "${newName}"`, true);
            imported.headers.push(newName);
            imported.data.forEach(row => {
    ...
    function _doDeleteColumns() {
        ...
        document.getElementById('dm-delcol-confirm').addEventListener('click', () => {
            ...
            if (!toDelete.length)
                return _showToast('⚠️ Selecciona al menos una columna', true);
            if (toDelete.length === imported.headers.length)
                return _showToast('⚠️ No puedes eliminar todas las columnas', true);
    ...
    function _doSearchReplace() {
        ...
        if (!find)
            return _showToast('⚠️ Escribe un valor a buscar', true);
        let n = 0;
