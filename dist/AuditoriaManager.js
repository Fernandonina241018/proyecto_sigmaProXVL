...
const AuditoriaManager = (() => {
    ...
    function filtrar({ module = '', usuario = '', desde = '', hasta = '', soloFallidos = false }) {
        _filteredLogs = _logs.filter(l => {
            if (module && l.module !== module)
                return false;
            if (usuario && !l.username.toLowerCase().includes(usuario.toLowerCase()))
                return false;
            if (soloFallidos && l.success !== 0)
                return false;
            if (desde) {
    ...
    function _renderAlertas() {
        ...
        container.innerHTML = alertas.map(a => `<div class="aud-alert aud-alert-${a.tipo.toLowerCase()}">⚠️ ${a.msg}</div>`).join('');
    }
    ...
    function _attachListeners() {
        ...
        document.getElementById('aud-btn-reset')?.addEventListener('click', () => {
            ...
            inputs.forEach(id => { const el = document.getElementById(id); if (el)
                el.value = ''; });
            const cb = document.getElementById('aud-filter-fallidos');
            if (cb)
                cb.checked = false;
            _filteredLogs = [..._logs];
            _renderTable(_logs);
        ...
        document.getElementById('aud-filter-user')?.addEventListener('keydown', (e) => {
            if (e.key === 'Enter')
                document.getElementById('aud-btn-filter')?.click();
        });
    }
