...
const EstadisticaDescriptiva = (() => {
    ...
    function calcularIntervalosConfianza(values) {
        ...
        if (n < 2)
            return { ic90: null, ic95: null, ic99: null };
    ...
    function tDistributionCDF(t, df) {
        ...
        const betaApprox = Math.exp(a * Math.log(x) + Math.log(sum) - Math.log(a) -
    ...
    function calcularKruskalWallis(grupos) {
        ...
        if (N < 5)
            return { error: 'Se necesitan al menos 5 observaciones en total' };
    ...
    function calcularWilcoxon(datos1, datos2) {
        ...
        if (n < 5)
            return { error: 'Se necesitan al menos 5 observaciones' };
    ...
    function calcularFriedman(tratamientos) {
        ...
        if (n < 3)
            return { error: 'Se necesitan al menos 3 bloques' };
    ...
    function calcularANOVA(grupos) {
        ...
        const N = grupos.reduce((sum, g) => sum + g.length, 0);
        if (N < k + 1)
            return { error: 'Datos insuficientes para ANOVA' };
    ...
    function calcularTestNormalidad(values) {
        const n = values.length;
        if (n < 8)
            return { error: 'Se necesitan al menos 8 observaciones' };
    ...
    function calcularShapiroWilk(values) {
        ...
        if (n > 5000)
            return { error: 'El test de Shapiro-Wilk está limitado a 5000 observaciones' };
    ...
    function calcularKolmogorovSmirnov(values) {
        ...
        if (std === 0)
            return { error: 'Desviación estándar cero (todos los valores iguales)' };
    ...
    function calcularCorrelacionPearson(x, y) {
        ...
        const denominator = Math.sqrt((nVal * sumX2 - sumX * sumX) * (nVal * sumY2 - sumY * sumY));
        if (denominator === 0) {
            return {
                r: 0,
                r2: 0,
    ...
    */
    ...
    function calcularRegresionMultiple(X, y) {
        ...
        // Predicciones
        const predicciones = XWithIntercept.map(row => row.reduce((sum, val, i) => sum + val * betas[i], 0));
        // Residuos
            ...
            switch (stat) {
                ...
                case 'Chi-Cuadrado':
                    ...
                    if (hypothesisConfig['Chi-Cuadrado']) {
                        ...
                        const tabla = valores1.map(v1 => valores2.map(v2 => data.data.filter(row => row[catCol1] === v1 && row[catCol2] === v2).length));
                ...
                case 'ANOVA Two-Way':
                    ...
                    if (hypothesisConfig['ANOVA Two-Way']) {
                        ...
                        const datos = factor1Labels.map(f1 => factor2Labels.map(f2 => {
                            const values = data.data
                ...
                case 'Límites de Cuantificación':
                    if (hypothesisConfig['Límites de Cuantificación']) {
                        ...
                        if (values.length === 0) {
                            ...
                            const cpk = std > 0 ?
                                Math.min((usl - media) / (3 * std), (media - lsl) / (3 * std)) : Infinity;
...
<div class="ar-kpi-card ar-kpi-multi ${statusClass}">
        ...
        function hypothesisKpiCards(statKey, data) {
            ...
            if (statKey === 'ANOVA One-Way') {
                if (data.error)
                    return `<p class="ar-error">${data.error}</p>`;
                return `
                    <div class="ar-kpi-card ar-kpi-multi">
            ...
            if (statKey === 'Regresión Lineal Múltiple') {
                if (data.error)
                    return `<p class="ar-error">${data.error}</p>`;
                const coefRows = data.coeficientes ? data.coeficientes.map(c => `<div class="ar-kpi-sub"><span class="ar-kpi-sub-k">${c.valor !== undefined ? 'β' + data.coeficientes.indexOf(c) : ''}</span><span class="ar-kpi-sub-v">${c.valor?.toFixed(4) ?? '—'}</span></div>`).join('') : '';
                return `
                    <div class="ar-kpi-card ar-kpi-multi">
        ...
        // ── Tags de columnas ───────────────────────────────────
        const colTags = cols.map(c => `<span class="ar-col-tag">${c}</span>`).join('');
        // ── Sección de parámetros de control ──────────────────
        let paramSection = '';
        if (hasParams) {
            const verifs = cols
                .map(col => ParametrosManager.verificarColumna(typeof getDataForModal === 'function' ? getDataForModal() : StateManager.getImportedData(), col))
                .filter(v => v !== null);
            if (verifs.length > 0) {
