...
const ReporteManager = (() => {
    ...
    function fmtNum(val, d = 4) {
        ...
        if (Array.isArray(val))
            return val.length ? val.map(v => Number(v).toFixed(d)).join('; ') : '—';
        if (typeof val === 'number')
            return isFinite(val) ? val.toFixed(d) : 'N/A';
        ...
        resultados.columnasAnalizadas.forEach(col => {
            ...
            if (media != null && sd != null && media !== 0) {
                ...
                if (cv > FLAGS.CV_VERY_HIGH)
                    flags.push(`[FLAG-CV-EXTREME] CV=${cv.toFixed(1)}% > ${FLAGS.CV_VERY_HIGH}%`);
                else if (cv > FLAGS.CV_HIGH)
                    flags.push(`[FLAG-CV-HIGH] CV=${cv.toFixed(1)}% > ${FLAGS.CV_HIGH}%`);
            }
            if (n < FLAGS.N_MIN)
                flags.push(`[FLAG-SMALL-N] n=${n} < ${FLAGS.N_MIN}`);
            if (q1 != null && q3 != null) {
                const iqr = q3 - q1;
                if (min < q1 - FLAGS.OUTLIER_IQR * iqr)
                    flags.push('[FLAG-OUTLIER-LOW] Mín bajo límite inferior');
                if (max > q3 + FLAGS.OUTLIER_IQR * iqr)
                    flags.push('[FLAG-OUTLIER-HIGH] Máx sobre límite superior');
            }
    ...
    function generarTXT(resultados, meta, hash) {
        ...
        resultados.columnasAnalizadas.forEach(col => {
            ...
            if (ext[col]?.paramVerificacion) {
                ...
                if (p_.min !== null)
                    p(`  ${pad('  Límite Mínimo :', 28)}${pad(String(p_.min), 18)}`);
                if (p_.max !== null)
                    p(`  ${pad('  Límite Máximo :', 28)}${pad(String(p_.max), 18)}`);
                if (p_.esp !== null)
                    p(`  ${pad('  Esperanza     :', 28)}${pad(String(p_.esp), 18)}`);
        ...
        if (hypResults.length > 0) {
            p(singleLine(W));
            p(`  ${lang === 'es' ? 'PRUEBAS DE HIPÓTESIS Y ESPECIFICACIÓN' : 'HYPOTHESIS AND SPECIFICATION TESTS'} (α = 0.05)`);
            p(singleLine(W));
            p('');
        ...
        resultados.columnasAnalizadas.forEach(col => {
            ...
            Object.entries(resultados.resultados).forEach(([stat, data]) => {
                ...
                if (val === undefined)
                    return;
                if (typeof val === 'object' && !Array.isArray(val)) {
                    Object.entries(val).forEach(([k, v]) => rows.push([col, stat, k, fmtNum(v, 6), cv, paramMin, paramMax, paramEsp, paramFuera, paramCumpl, flagCount, flagsTxt].join('|')));
                }
    ...
    function generarHTML(resultados, meta, hash) {
        ...
        function statsRows(col) {
            ...
            Object.entries(resultados.resultados).forEach(([stat, data]) => {
                if (hypothesisTests.has(stat))
                    return; // Skip hypothesis tests - shown separately
                const val = data[col];
                if (val === undefined)
                    return;
                if (typeof val === 'object' && !Array.isArray(val)) {
            ...
            if (pv) {
                ...
                if (p_.min !== null)
                    h += `<tr><td style="padding-left:26px;color:#555">· Límite Mínimo</td><td style="font-family:monospace;text-align:right;color:#3730a3;font-weight:500">${p_.min}</td><td style="color:#a0aec0;font-size:8.5pt;font-style:italic;text-align:right">Definido por usuario</td></tr>`;
                if (p_.max !== null)
                    h += `<tr><td style="padding-left:26px;color:#555">· Límite Máximo</td><td style="font-family:monospace;text-align:right;color:#3730a3;font-weight:500">${p_.max}</td><td style="color:#a0aec0;font-size:8.5pt;font-style:italic;text-align:right">Definido por usuario</td></tr>`;
                if (p_.esp !== null)
                    h += `<tr><td style="padding-left:26px;color:#555">· Esperanza</td><td style="font-family:monospace;text-align:right;color:#3730a3;font-weight:500">${p_.esp}</td><td style="color:#a0aec0;font-size:8.5pt;font-style:italic;text-align:right">Definido por usuario</td></tr>`;
        ...
        function flagBadges(col) {
            if (!ext[col]?.flags?.length)
                return `<span style="background:#c6f6d5;color:#276749;font-family:monospace;font-size:7.5pt;padding:3px 8px;border-radius:3px">${t('html_noFlags')}</span>`;
        ...
        const sigBlocks = roleKeys.map((k, i) => {
            ...
            return `<div style="border:1px solid #e2e8f0;border-radius:6px;padding:14px">
              ...
              ${[[t('name'), name], [t('title'), title || '—'], [t('date'), date]].map(([l, v]) => `<div style="margin-bottom:8px"><span style="font-size:7pt;color:#a0aec0;font-family:monospace;text-transform:uppercase;display:block">${l}</span><span style="font-size:9.5pt;border-bottom:1px solid #e2e8f0;padding-bottom:3px;display:block;color:${!v || v === '—' ? '#cbd5e0' : '#1a202c'};${!v || v === '—' ? 'font-style:italic' : ''}">${v || ''}</span></div>`).join('')}
  ...
  <div class="doc-footer">
    ...
    async function descargar(formatos, resultados, meta) {
        ...
        if (formatos.includes('html')) {
            setTimeout(() => downloadBlob(generarHTML(resultados, meta, hash), `${base}.html`, 'text/html;charset=utf-8'), delay);
            delay += 350;
        }
        if (formatos.includes('pdf')) {
            setTimeout(() => { const html = generarHTML(resultados, meta, hash); const w = window.open('', '_blank'); w.document.write(html); w.document.close(); w.print(); }, delay);
            delay += 350;
        }
        if (formatos.includes('txt')) {
            setTimeout(() => downloadBlob(generarTXT(resultados, meta, hash), `${base}.txt`, 'text/plain;charset=utf-8'), delay);
            delay += 350;
        }
        if (formatos.includes('csv')) {
            setTimeout(() => downloadBlob(generarCSV(resultados, meta, hash), `${base}.csv`, 'text/csv;charset=utf-8'), delay);
        }
    ...
    function buildReportesView() {
        ...
        document.getElementById('rep-btn-download')?.addEventListener('click', async () => {
            ...
            if (!formatos.length) {
                alert(t('ui_alertNoFmt'));
                return;
            }
            const meta = collectMeta();
            const result = await descargar(formatos, resultados, meta);
            setTimeout(() => alert(t('ui_alertDownload', result.formatos.map(f => `.${f.toUpperCase()}`).join(' · '), result.base, formatos.includes('html'))), formatos.length * 350 + 100);
        });
    }
    ...
    function updateFmtCount() {
        ...
        if (el)
            el.textContent = t('ui_formatCount', n);
    }
