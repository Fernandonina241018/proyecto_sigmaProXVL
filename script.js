...
function openNormalDataModal() {
    document.getElementById('normal-data-modal').style.display = 'flex';
}

function closeNormalDataModal() {
    document.getElementById('normal-data-modal').style.display = 'none';
}
...
function generateNormalData() {
    ...
    const stdDev = parseFloat(document.getElementById('nd-stddev').value);
    ...
    if (isNaN(min) || isNaN(max) || isNaN(stdDev) || isNaN(decimals) || isNaN(rows) || isNaN(cols)) {
        alert('⚠️ Todos los campos deben ser números válidos');
        return;
    }
    if (max <= min) {
    ...
    if (rows < 1 || rows > 100 || cols < 1 || cols > 50) {
        alert('⚠️ Las filas deben ser 1-100 y las columnas 1-50');
        return;
    }
    ...
    StateManager.createSheet('Datos Normales', rows, cols, headers, data);
    renderWorkTable();
    updateWorkSummary();
    renderSheetTabs();
    updateSheetsInfo();
    closeNormalDataModal();
    showToast('✅ Datos normales generados: ' + rows + '×' + cols);
}
