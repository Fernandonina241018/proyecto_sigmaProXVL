...
interface OutlierResultBase {
  outliers: number[];
  cantidad: number;
  porcentaje: number;
}

interface OutlierIQRResult extends OutlierResultBase {
  limiteInferior: number;
  limiteSuperior: number;
  porcentajeOutliers: string;
  detalles: Array<{ index: number; value: number; method: string }>;
}

interface OutlierZScoreResult extends OutlierResultBase {
  umbralZScore: number;
  porcentajeOutliers: string;
  detalles: Array<{ index: number; value: number; zScore: number; method: string }>;
}

interface ResumenEstadistico {
  n: number;
  media: number;
  mediana: number;
  moda: number[];
...
const StatsUtils = (function () {
   ...
   */
  function getNumericValues(data: Dataset, colName: string): number[] {
   ...
   */
  function getNumericColumns(data: Dataset, options: NumericColumnOptions = {}): string[] {
    if (!data || !data.headers || !data.data) return [];
   ...
   */
  function countMissingValues(data: Dataset): number {
    if (!data || !data.headers || !data.data) return 0;
    let count = 0;
    data.data.forEach(row => {
      data.headers.forEach((h, idx) => {
        const val = Array.isArray(row) ? row[idx] : row[h];
        if (val === null || val === '' || val === undefined || isNaN(parseFloat(String(val)))) count++;
      });
    });
    return count;
  }
   ...
   * Calcula la moda
   */
  function calcularModa(values: number[]): number[] {
   ...
   * Calcula la varianza
   */
  function calcularVarianza(values: number[], esMuestral = true): number {
    if (!values || values.length === 0) return 0;
   ...
   */
  function calcularDesviacionEstandar(values: number[], esMuestral = true): number {
   ...
   * Calcula el rango
   */
  function calcularRango(values: number[]): number {
    if (!values || values.length === 0) return 0;
   ...
   */
  function calcularCoeficienteVariacion(values: number[]): number {
    const mean = calcularMedia(values);
    if (mean === 0) return 0;
   ...
   */
  function calcularPercentil(values: number[], p: number): number {
    if (!values || values.length === 0) return 0;
   ...
   */
  function calcularCuartiles(values: number[]): Cuartiles {
    return {
   ...
   */
  function calcularIQR(values: number[]): number {
   ...
   */
  function calcularAsimetria(values: number[], esMuestral = true): number {
   ...
   * Calcula la curtosis
   */
  function calcularCurtosis(values: number[], esMuestral = true): number {
   ...
   */
  function calcularCovarianza(x: number[], y: number[]): number {
   ...
   */
  function detectarOutliersIQR(values: number[], options: OutlierOptions = {}): unknown[] | OutlierIQRResult {
    const emptySimple: unknown[] = [];
    ...
    const outliers: Array<{ index: number; value: number; method: string }> = [];
    values.forEach((v, i) => {
      if (v < lower || v > upper) {
   ...
   */
  function detectarOutliersZScore(values: number[], options: OutlierOptions = {}): unknown[] | OutlierZScoreResult {
    const emptySimple: unknown[] = [];
    ...
    const outliers: Array<{ index: number; value: number; zScore: number; method: string }> = [];
    values.forEach((v, i) => {
   ...
   */
  function calcularErrorEstandar(values: number[]): number {
   ...
   */
  function resumenEstadistico(values: number[]): ResumenEstadistico | { error: string } {
