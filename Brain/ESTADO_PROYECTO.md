...
| 57 | 21 May 2026 | Fix: Color gris en datos de firma 🔴 — `firmaUpdatePreview()` solo actualizaba `textContent` pero el `color:#cbd5e0` quedaba fijo del HTML generado. Ahora también cambia `style.color` a `#1a202c` (negro) y `fontStyle` a `normal` al firmar. `indexx.js:3566-3581`. | ✅ |
| 58 | 21 May 2026 | Feat: Validación ICH Q2(R1) para Regresión Lineal Simple — `calcularRegresionLinealSimple()` ahora retorna IC 95% intercepto (`icInterceptLower`/`icInterceptUpper`), banderas `cumpleICH`/`cumpleFDA`, tabla `desviaciones[]` por nivel con `maxDesviacion`. UI muestra tarjeta ICH con badges (✓/✗), tablas expandibles de desviaciones y residuales. Reporte TXT y HTML incluyen estado ICH/FDA. `EstadisticaDescriptiva.js:1995-2052,5410-5483`, `estadisticosConfig.js:1258`, `ReporteManager.js:904-917,1666-1685`. | ✅ |
| 59 | 21 May 2026 | Feat: Gráfico de Linealidad en Visualización — nuevo tipo "Linealidad" (∎) muestra scatter + recta de regresión con fórmula y R² en overlay superior derecho. `Visualizacion.js`, `visualizacion.css`. | ✅ |
