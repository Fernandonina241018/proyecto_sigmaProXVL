...
const ReporteManager = (() => {
    ...
    function computeExtendedStats(resultados) {
        ...
        resultados.columnasAnalizadas.forEach(col => {
            ...
            // ── Parámetros de control (LSL/USL/Target) ──
            let compliance = null;
            if (typeof ParametrosManager !== 'undefined') {
                const imported = typeof StateManager !== 'undefined' ? StateManager.getImportedData() : null;
                if (imported) {
                    const verif = ParametrosManager.verificarColumna(imported, col);
                    if (verif) {
                        const pct = parseFloat(verif.porcentajeCumplimiento);
                        compliance = verif;
                        if (verif.fueraDeRango > 0) {
                            flags.push(`[FLAG-OUT-OF-SPEC] ${verif.fueraDeRango}/${verif.total} valores fuera de especificación (${(100-pct).toFixed(1)}%)`);
                        }
                        if (verif.parametros.esp !== null && verif.mediaReal !== null) {
                            const diff = Math.abs(verif.mediaReal - verif.parametros.esp);
                            const pctDiff = verif.parametros.esp !== 0 ? (diff / Math.abs(verif.parametros.esp) * 100) : 0;
                            if (pctDiff > 5) flags.push(`[FLAG-TARGET-DRIFT] Media ${pctDiff.toFixed(1)}% desviada del Target`);
                        }
                    }
                }
            }
            ext[col]={cv,flags,notes,compliance};
        });
        return ext;
    }
    ...
    function generarTXT(resultados, meta) {
        ...
        resultados.columnasAnalizadas.forEach(col=>{
            ...
            if(ext[col]?.compliance) {
                const c=ext[col].compliance; const pp=c.parametros;
                p('');p(`  ── ${currentLang==='es'?'PARÁMETROS DE CONTROL':'CONTROL PARAMETERS'} ──`);
                if(pp.min!==null) p(`  ${pad('LSL:',28)}${pp.min}`);
                if(pp.max!==null) p(`  ${pad('USL:',28)}${pp.max}`);
                if(pp.esp!==null) p(`  ${pad('Target:',28)}${pp.esp}`);
                p(`  ${pad((currentLang==='es'?'Cumplimiento':'Compliance')+':',28)}${c.porcentajeCumplimiento}%  (${c.fueraDeRango}/${c.total} ${currentLang==='es'?'fuera de rango':'out of range'})`);
            }
    ...
    function generarHTML(resultados, meta) {
        ...
        function statsRows(col){
            ...
            if(ext[col]?.compliance) {
                const c = ext[col].compliance;
                const p = c.parametros;
                const pct = parseFloat(c.porcentajeCumplimiento);
                const pctColor = pct>=99?'#276749':pct>=90?'#b7791f':'#c53030';
                const pctBg    = pct>=99?'#f0fff4':pct>=90?'#fffbeb':'#fff5f5';
                h+=`<tr style="background:#f0f4ff"><td colspan="3" style="padding:5px 14px;font-size:8.5pt;color:#1a3a6b;font-weight:600">⚙ ${lang==='es'?'Parámetros de Control':'Control Parameters'}</td></tr>`;
                if(p.min!==null) h+=`<tr><td style="padding-left:20px">LSL (${lang==='es'?'Mínimo':'Minimum'})</td><td style="font-family:monospace;text-align:right;font-weight:500;color:#2c5282">${p.min}</td><td style="color:#a0aec0;font-size:8.5pt;text-align:right">${lang==='es'?'Límite inferior':'Lower spec limit'}</td></tr>`;
                if(p.max!==null) h+=`<tr><td style="padding-left:20px">USL (${lang==='es'?'Máximo':'Maximum'})</td><td style="font-family:monospace;text-align:right;font-weight:500;color:#2c5282">${p.max}</td><td style="color:#a0aec0;font-size:8.5pt;text-align:right">${lang==='es'?'Límite superior':'Upper spec limit'}</td></tr>`;
                if(p.esp!==null) h+=`<tr><td style="padding-left:20px">Target</td><td style="font-family:monospace;text-align:right;font-weight:500;color:#2c5282">${p.esp}</td><td style="color:#a0aec0;font-size:8.5pt;text-align:right">${lang==='es'?'Valor objetivo':'Target value'}</td></tr>`;
                h+=`<tr style="background:${pctBg}"><td><strong>${lang==='es'?'Cumplimiento':'Compliance'}</strong></td><td style="font-family:monospace;text-align:right;font-weight:700;color:${pctColor}">${pct}%</td><td style="text-align:right;font-size:8.5pt;color:${pctColor}">${c.fueraDeRango}/${c.total} ${lang==='es'?'fuera de rango':'out of range'}</td></tr>`;
            }
            return h;
        }
